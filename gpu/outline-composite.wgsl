...
struct Params {
  ...
  thickness: f32,
  opacity: f32,
  position: f32, // 0 = inside, 0.5 = center, 1 = outside
  threshold: f32,
}
...
fn main(@builtin(global_invocation_id) gid: vec3u) {
  ...
  // Proper signed distance: negative inside, positive outside
  var signedDist: f32;
  if (isInside) {
  ...
  // Preserve alpha: keep original alpha, but extend with outline
